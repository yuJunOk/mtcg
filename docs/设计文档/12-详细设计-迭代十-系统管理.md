...
在迭代一用户系统与权限管理基础上，实现系统管理员的后台管理能力：运行参数配置、完善操作审计日志（用户 CRUD 已在迭代一交付）。本迭代为运营与审计提供闭环，不引入新通信模式，全部基于 REST。

> **as-built 前置**：迭代二已建 `mtcg_audit_log` 并落地 `AuditService` 雏形。本迭代应在此表/服务上扩展（查询 API、`@LogOperation` AOP、覆盖面），避免再引入平行的 `operation_log` 表；下文 `operation_log` 设计视为历史草案，实现时对齐 `mtcg_audit_log` 字段语义。

### 1.2 范围

| 包含 | 不包含 |
| --- | --- |
| `system_config` 建表；完善 `mtcg_audit_log`（原草案名 `operation_log`） | 细粒度权限 ACL |
...
| 依赖 | 来源 | 复用内容 |
| --- | --- | --- |
| 用户系统 | 迭代一 | `UserDO` / `UserMapper` / `EnumUserStatus` / `EnumUserRole`；用户管理 CRUD 已交付 |
| 权限控制 | 迭代一 | `@RequireRole(EnumUserRole.SYS_ADMIN)`、`JwtInterceptor`、`SecurityConstant` |
| 审计雏形 | 迭代二 | `mtcg_audit_log` / `AuditService`（本迭代扩展） |
| 统一响应 | 迭代一 | `Result<T>` / `PageVO<T>` / `ErrorCode` / `BusinessException` / `GlobalExceptionHandler` |
...
> 以下接口由本迭代交付，供后续迭代（暂无）调用者使用。本迭代为整个项目的最后阶段，无后续迭代依赖。

| 公开方法 | 签名 | 调用方 | 用途 |
| --- | --- | --- | --- |
| `SystemConfigService.getString(key, defaultVal)` | `String` | 任意模块 | 读取运行参数 |
| `SystemConfigService.getInt(key, defaultVal)` | `int` | 任意模块 | 读取运行参数（整数） |
| `SystemConfigService.setValue(key, value)` | `void` | 任意模块 | 修改运行参数（需 SYS_ADMIN） |
| `@LogOperation` | 注解 | 所有业务方法 | 自动记录审计日志 |

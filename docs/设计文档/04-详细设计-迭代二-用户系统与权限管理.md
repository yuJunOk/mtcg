...
在迭代一卡牌数据落地基础上，实现用户注册/登录、个人资料管理（含系统头像）、消息通知，以及基于 JWT + 角色注解的接口鉴权与权限隔离，为后续卡组归属、对局归属提供用户基座。
...
| `mtcg_system_avatar` 表（系统预设头像） | 对局记录（迭代七） |
| `mtcg_notification` 表（消息通知） | 排位、系统管理（迭代九/十） |
| `EnumUserRole` / `EnumUserStatus` 枚举 | 细粒度权限（资源级 ACL） |
| UserDO / DTO / VO 实体 | OAuth / 第三方登录 |
| SystemAvatarDO / NotificationDO 实体 | 私信/聊天模块 |
| UserMapper（MyBatis-Flex） | WebSocket 鉴权 |
| JwtManager（Token 生成/验证） | |
| UserService（注册/登录/资料查改/头像选择/通知查询） | |
| AuthController / UserController / AvatarController / NotificationController | |
...
| FR5.3 | 用户资料（昵称/头像） | `GET/PUT /users/me`；`GET /avatars` 获取系统头像列表；`POST /users/me/avatar` 上传自定义头像 |
| FR5.8 | 消息通知 | `GET /notifications` 查询通知列表；`PUT /notifications/{id}/read` 标记已读；系统内部触发通知创建 |
...
支持两种头像来源：
- **系统预设头像**：提供一批游戏主题的预设头像，用户直接选择，无需上传
- **自定义上传**：用户可上传自定义图片，存储到 OSS/本地，URL 存入 `avatar` 字段

### 12.2 建表 SQL

```sql
-- 系统预设头像表
CREATE TABLE mtcg_system_avatar (
    id              BIGSERIAL       PRIMARY KEY,
    avatar_name     VARCHAR(64)     NOT NULL,          -- 头像名称（如 "钢铁侠"）
    avatar_url      VARCHAR(256)    NOT NULL,          -- 头像图片 URL
    sort_order      INT             NOT NULL DEFAULT 0,-- 排序（数值越小越靠前）
 ...
 * 头像来源枚举
 */
public enum EnumAvatarSource implements IEnum<String> {
    SYSTEM("SYSTEM", "系统预设"),
    CUSTOM("CUSTOM", "自定义上传");
    ...
    EnumAvatarSource(String code, String desc) {
        this.code = code;
        this.desc = desc;
    }
...
@Data
@Table("mtcg_system_avatar")
public class SystemAvatarDO {
    private Long id;
...
| 方法 | 路径 | 说明 | 鉴权 |
| --- | --- | --- | --- |
| GET | `/api/v1/avatars` | 获取系统预设头像列表 | 需登录 |
| POST | `/api/v1/users/me/avatar` | 上传自定义头像 | 需登录 |
| PUT | `/api/v1/users/me/avatar/{avatarId}` | 选择系统头像 | 需登录 |

**GET /api/v1/avatars** 返回示例：

```json
{
  "code": 0,
  "data": [
    { "id": 1, "avatarName": "钢铁侠", "avatarUrl": "/assets/avatars/ironman.png" },
    { "id": 2, "avatarName": "美国队长", "avatarUrl": "/assets/avatars/captain.png" }
  ]
}
```

**POST /api/v1/users/me/avatar** 上传自定义头像：
- 请求：`multipart/form-data`，字段 `file`
- 限制：文件大小 ≤ 2MB，格式 jpg/png/webp
- 处理：上传到 OSS/本地存储，返回 URL，更新 `mtcg_user.avatar`

**PUT /api/v1/users/me/avatar/{avatarId}** 选择系统头像：
- 校验 `avatarId` 对应的系统头像存在且启用
- 更新 `mtcg_user.avatar` 为系统头像 URL
...
```sql
-- 消息通知表
CREATE TABLE mtcg_notification (
    id              BIGSERIAL       PRIMARY KEY,
    user_id         BIGINT          NOT NULL,          -- 接收用户 ID
    type            VARCHAR(32)     NOT NULL,          -- 通知类型：MATCH_FOUND/FRIEND_REQUEST/SYSTEM_NOTICE/REWARD/TURN_REMIND
    title           VARCHAR(128)    NOT NULL,          -- 通知标题
    content         VARCHAR(512)    NOT NULL,          -- 通知内容
    ref_id          BIGINT,                            -- 关联业务 ID（如对局 ID、请求 ID）
...
-- 索引
CREATE INDEX idx_notification_user_read ON mtcg_notification(user_id, is_read, create_time DESC);
```
...
public enum EnumNotificationType implements IEnum<String> {
    MATCH_FOUND("MATCH_FOUND", "匹配成功"),
    FRIEND_REQUEST("FRIEND_REQUEST", "好友请求"),
    SYSTEM_NOTICE("SYSTEM_NOTICE", "系统公告"),
    REWARD("REWARD", "奖励发放"),
    TURN_REMIND("TURN_REMIND", "回合提醒");

    private final String code;
    private final String desc;

    EnumNotificationType(String code, String desc) {
        this.code = code;
        this.desc = desc;
    }
...
| 方法 | 路径 | 说明 | 鉴权 |
| --- | --- | --- | --- |
| GET | `/api/v1/notifications` | 查询当前用户通知列表（分页） | 需登录 |
| GET | `/api/v1/notifications/unread-count` | 获取未读通知数量 | 需登录 |
| PUT | `/api/v1/notifications/{id}/read` | 标记单条通知已读 | 需登录 |
| PUT | `/api/v1/notifications/read-all` | 全部标记已读 | 需登录 |

**GET /api/v1/notifications** 返回示例：
    ...
    "list": [
      {
        ...
        "type": "MATCH_FOUND",
        "title": "匹配成功",
        "content": "已为您匹配到对手「PlayerX」，段位：白银 II",
        "refId": 12345,
        "isRead": false,
        "createTime": "2026-08-02T12:00:00"
      }
    ]
  }
}
...
通知由后端业务逻辑内部触发创建，不通过外部 API 创建：

| 触发场景 | 触发位置 | 通知类型 |
| --- | --- | --- |
| 匹配成功 | 匹配服务（迭代九） | `MATCH_FOUND` |
| 轮到当前玩家操作 | 对战引擎 | `TURN_REMIND` |
| 收到好友请求 | 好友服务（后续） | `FRIEND_REQUEST` |
| 管理员发布公告 | 系统管理（迭代十） | `SYSTEM_NOTICE` |
| 奖励发放 | 赛季结算/任务系统 | `REWARD` |
...
public class NotificationService {
    ...
    public void createNotification(Long userId, EnumNotificationType type, 
...
| 16 | `AvatarController.java` | `controller` | 头像列表/选择/上传接口 |
| 17 | `NotificationController.java` | `controller` | 通知查询/标记已读接口 |
| 18 | `RequireRole.java` | `common/annotation` | 角色控制注解 |
| 19 | `SecurityConstant.java` | `common/constant` | 安全相关常量 |
| 20 | `JwtInterceptor.java` | `component` | JWT 鉴权拦截器 |
| 21 | `WebMvcConfig.java` | `config` | 注册拦截器 |
| 22 | `EnumAvatarSource.java` | `common/enums` | 头像来源枚举 |
| 23 | `EnumNotificationType.java` | `common/enums` | 通知类型枚举 |
| 24 | `SystemAvatarDO.java` | `domain/entity` | 系统头像实体 |
| 25 | `NotificationDO.java` | `domain/entity` | 通知实体 |
| 26 | `SystemAvatarMapper.java` | `dao` | 系统头像 Mapper |
| 27 | `NotificationMapper.java` | `dao` | 通知 Mapper |
| 28 | `NotificationService.java` | `service` | 通知创建服务 |
| 29 | `user.sql` | `resources/sql`（追加到 init.sql） | user/avatar/notification 表建表 SQL |

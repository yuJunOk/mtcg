> **迭代规划调整说明（2026-08-03）**：本文档原对应旧迭代二，按 MVP 优先原则重新规划后，对应【新迭代一：基础设施 + 用户系统（含用户管理）】。已合并旧迭代十的用户管理 CRUD 部分；头像系统与消息通知移至新迭代九。详见 [需求分析 §6](./01-需求分析.md#6-需求优先级与迭代规划)。文件名保留历史编号不变。
...
| 包含 | 不包含 |
| --- | --- |
| `mtcg_user` / `mtcg_card` / `mtcg_product` / `mtcg_product_category` 四张表建表 | 卡牌/产品 CRUD 接口（迭代二） |
...
| D8 | 四张表一次建完 | `mtcg_card` / `mtcg_product` / `mtcg_product_category` 虽迭代二才用，但提前建表避免后续 DDL 变更 |
...
-- =====================================================
-- 产品表（迭代二使用，本迭代先建空表）
-- product_type 改为 category_code，引用 mtcg_product_category（应用层校验，不建外键）
-- =====================================================
CREATE TABLE IF NOT EXISTS mtcg_product (
    id              BIGSERIAL       PRIMARY KEY,
    product_code    VARCHAR(16)     NOT NULL UNIQUE,
    product_name    VARCHAR(128)    NOT NULL,
    category_code   VARCHAR(32)     NOT NULL,          -- 引用 mtcg_product_category.category_code
    release_date    DATE,
    description     TEXT,
    create_time     TIMESTAMP       NOT NULL DEFAULT NOW(),
    update_time     TIMESTAMP       NOT NULL DEFAULT NOW()
...
-- =====================================================
-- 产品分类表（可扩展的发售分类，替代原固定枚举）
-- =====================================================
CREATE TABLE IF NOT EXISTS mtcg_product_category (
    id              BIGSERIAL       PRIMARY KEY,
    category_code   VARCHAR(32)     NOT NULL UNIQUE,   -- 分类编码，如 STRUCTURE_DECK / BOOSTER_PACK
    category_name   VARCHAR(64)     NOT NULL,           -- 分类名称，如 预组牌 / 补充包
...
-- 初始化默认分类（后续可由管理后台增删改）
INSERT INTO mtcg_product_category (category_code, category_name, sort_order) VALUES
    ('STRUCTURE_DECK', '预组牌', 1),
    ('BOOSTER_PACK',   '补充包', 2),
    ('PROMO_PACK',     '推广包', 3)
ON CONFLICT (category_code) DO NOTHING;

-- =====================================================
-- 触发器：update_time 自动更新（四表共用一个函数）
-- =====================================================
...
DROP TRIGGER IF EXISTS trigger_product_category_update_time ON mtcg_product_category;
CREATE TRIGGER trigger_product_category_update_time
    BEFORE UPDATE ON mtcg_product_category FOR EACH ROW EXECUTE FUNCTION update_update_time();
```
...
`EnumCardType`（CHARACTER/RUSH_POINT）、`EnumColor`（6 色）、`EnumRarity`（12 种）—— 结构同上，详见 [迭代二详细设计](./03-详细设计-迭代一-卡牌数据落地.md) §3。

> **注意**：产品类型不再使用固定枚举（原 `EnumProductType` 已移除），改为 `mtcg_product_category` 分类表管理，详见迭代二设计。
...
| 6 | ~~`EnumProductType.java`~~ | ~~`common/enums`~~ | **已移除**：产品类型改为 `mtcg_product_category` 分类表管理 |
...
| `init.sql` | 重写四张表（mtcg_user / mtcg_card / mtcg_product / mtcg_product_category）+ 触发器 |

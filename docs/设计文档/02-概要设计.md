...
| `mtcg_product` | 卡包/商品系列表 | `product_code` 唯一标识（如 SD01/BP01/PB01）；卡牌通过 `product_code` 归属卡包 |
...
// 产品（卡包/商品系列）由 mtcg_product 表管理，不使用枚举
...
| **枚举** | `EnumCardType`、`EnumColor`、`EnumRarity`、**`EnumRole`**（PLAYER/CARD_ADMIN/SYS_ADMIN/AI）；产品（卡包）由 `mtcg_product` 表管理，不使用枚举 |

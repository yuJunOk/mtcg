...
| 游戏渲染 | PixiJS 8 | WebGL 2D 渲染引擎，卡牌 Sprite、拖拽、动画 |
| 状态管理 | Pinia | 响应式对局状态，与后端 GameState 结构对应 |
| 动画 | GSAP（可选） | 复杂编排动画（入场序列、攻击连锁） |
| HTTP | Axios | REST API 调用 |
| 构建 | Vite 5 | 极速 HMR |
| 包管理 | npm workspaces | Monorepo 管理 common/game-pc/game-mobile 三个包 |
| PC 打包 | Electron | 打包为 Windows exe |
| 移动打包 | Capacitor | 打包为 Android apk / iOS ipa |
...
│   │       ├── api/client.ts        ← Axios 封装（/api/v1/games 等）

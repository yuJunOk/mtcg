> **迭代规划调整说明（2026-08-03）**：本文档原对应旧迭代一，按 MVP 优先原则重新规划后，对应【新迭代二：卡牌与产品管理】。建表 SQL 已移至迭代一统一执行；本迭代聚焦卡牌/产品的 CRUD 接口与管理后台页面对接。鉴权依赖迭代一的 `@RequireRole` 机制。详见 [需求分析 §6](./01-需求分析.md#6-需求优先级与迭代规划)。文件名保留历史编号不变。
...
> 版本：v0.2
> 日期：2026-08-03
> 依据：[需求分析](./01-需求分析.md) FR1.1–FR1.5、[概要设计](./02-概要设计.md) §3 §4 §5 §8、[迭代一详细设计](./04-详细设计-迭代二-用户系统与权限管理.md)（用户系统、`@RequireRole`）

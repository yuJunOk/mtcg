...
│   ├── ActionPhaseHandler               // 行动阶段处理器（303.2.a.3）
...
### 4.5 ActionPhaseHandler — 行动阶段（303.2.a.3）
...
classDiagram
    ...
    PhaseHandler <|.. ActionPhaseHandler
...
| `ActionPhaseHandler.java` | `engine.phase` | 行动阶段处理器（303.2.a.3） |
...
> 以下接口由本迭代交付，供后续迭代（迭代五 行动与战斗）调用者使用。

| 公开方法 | 签名 | 调用方 | 用途 |
| --- | --- | --- | --- |
| `GameState.getActivePlayer()` | `PlayerState` | 迭代五 CombatHandler | 获取当前回合玩家 |
| `GameState.getInactivePlayer()` | `PlayerState` | 迭代五 CombatHandler | 获取防守方玩家 |
| `GameState.getFirstPlayer()` | `PlayerState` | 迭代五 CombatHandler | 判断先攻首回合跳过战斗 |
| `GameState.switchActivePlayer()` | `void` | 迭代五 TurnEndHandler | 切换回合玩家 |
| `PlayerState.getField()` | `FieldZone` | 迭代五 SummonHandler | 战场部署位置校验 |
| `FieldZone.isBaseFull()` | `boolean` | 迭代五 SummonHandler | 基地是否已满（302.6 上限 6） |
| `CardInstance.resetTurnFlags()` | `void` | 迭代五 TurnEndHandler | 回合结束重置本回合标记 |
| `PhaseType.next()` | `PhaseType` | 迭代五 所有 Handler | 推进到下一阶段 |
| `RuleConstants.DRAW_PER_TURN` | `int = 2` | 迭代五 DrawHandler | 每回合抽卡数 |
| `RuleConstants.MAIN_DECK_SIZE` | `int = 50` | 迭代五 GameInitializer | 校验主卡组张数 |

> **引擎零侵入约定**：调用方为 Spring 层（迭代五~十），引擎本身不感知调用者身份。所有交互通过 `GameState` 对象进行，不涉及 Spring 容器。
...
| 需求 | 验收项 |
| --- | --- |
| FR3.1 | `GameInitializer` 正确初始化：双方各 50+9 张卡组实例创建完毕 |
| FR3.1 | 洗牌：双方 deck 无序，打印验证非固定顺序 |
| FR3.1 | 发起始手牌：双方各 6 张，deck 剩余 44+9 张 |
| FR3.1 | Mulligan：弃牌置卡组底 → 补等量 → 洗匀；先攻先决定，后攻后决定 |
| FR3.2 | 6 阶段严格顺序：`TURN_START → DRAW → ACTION → COMBAT → RESPONSE → TURN_END → TURN_START` |
| FR3.2 | 先攻首回合跳过战斗：`CombatHandler` 进入时检测 `turnCount==1 && active==first` → 直接推进到 RESPONSE |
| FR3.2 | 回合结束：手牌 > 9 → 舍弃末尾至 9 张；场上标记重置；玩家切换 |
| FR3.2 | 抽卡：`DrawHandler` 每回合抽 2 张；抽 2 张到手牌，deck 减少 |
| NFR1 | `engine` 包不含任何 Spring/MyBatis 注解或 import |
| NFR5 | 所有公开方法注释标注规则条款编号（如 `// 303.2.a.2`） |
| NFR5 | `RuleConstants` 包含全部常量，注释标注规则条款 |

---

## 9. Q&A 判例补充

> 以下判例来自 [规则 Q&A 第一期](../规则文档/05-规则Q&A-第1期.md)，影响本迭代的状态模型与回合流程设计，实现时需覆盖。

### 9.1 应对号召的目标区域含基地区（Q&A Q1）

**判例要点**：应对号召放置进入场上的角色卡可以放置于战区**或基地区**（102.2 场上含战区和基地区）。

**影响点**：`FieldZone` 的 `isOnField()` 已正确覆盖战区+基地，但迭代五的应对号召 Handler 校验目标区域时，不能仅检查战区空位，还需检查基地区是否已满（`isBaseFull()`，302.6 上限 6）。

### 9.2 Lv4+ 号召可同时使用战区和基地区的角色（Q&A Q7）

**判例要点**：Lv4+ 号召撤退角色时，可混合使用战区角色和基地区角色，只要合计 Lv 满足要求。

**影响点**：`CardInstance` 的运行时状态 `currentZone` 需能区分战区与基地区，`Zone.isOnField()` 已覆盖。迭代五 `SummonHandler.computeRetrivableLevel()` 需遍历**整个 `FieldZone`**（战区 + 基地区）统计可撤退角色的合计 Lv，不能仅遍历战区。

### 9.3 本回合生效的效果回合结束终止（Q&A Q2）

**判例要点**：本回合生效的效果（如战力增减）在回合结束时终止（303.2.a.6.1.2），不会带进对方回合。

**影响点**：`TurnEndHandler` 的步骤 2「终止本回合有效效果」（当前为 TODO 迭代六）需在迭代六实现时，清理所有 `Duration.THIS_TURN` 的 `PowerModifier`。本迭代 `TurnEndHandler.resetTurnFlags()` 已重置回合标记，效果清理在迭代六 `EffectResolver` 接入后补充。

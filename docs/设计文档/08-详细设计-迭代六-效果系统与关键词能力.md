...
> 前置依赖：迭代四（状态模型 + 回合流程）、迭代五（行动/战斗处理器 ActionDispatcher）已就绪
...
| `ActionTypeHandler`（ACTIVATE_EFFECT） | 迭代五 | 启动型效果入口 |
...
public class ResponseHandler implements KeywordHandler {
    ...
    public void apply(EffectContext ctx) {
        // 标记此卡可作为应对号召对象；实际号召流程由 ActionTypeHandler（RESPONSE_SUMMON）执行
...
```mermaid
sequenceDiagram
    participant AH as ActionTypeHandler
    participant ER as EffectResolver
...
> 以下接口由本迭代交付，供后续迭代（迭代七 AI 对战）调用者使用。

| 公开方法 | 签名 | 调用方 | 用途 |
| --- | --- | --- | --- |
| `EffectContext.source` | `CardInstance` | 迭代七 AI（局面评估） | 读取效果源头卡 |
| `EffectContext.gameState` | `GameState` | 迭代七 AI | 读取全局状态 |
| `CardInstance.getKeywords()` | `Set<Keyword>` | 迭代七 AI（评估威胁） | 识别关键词能力 |
| `EffectResolver.resolve(state, trigger)` | `void` | 迭代七 AI（作弊防护） | 确认效果已结算 |
| `PowerModifierStack.compute(card, attr)` | `int` | 迭代七 AI（战力计算） | 正确计算当前战力 |
| `PhaseHandler.onEnter(state, engine)` | `void` | 迭代七 AI | 驱动回合推进 |

> **挂接约定**：AI 对战时，`AIManager` 在 `GameService.createGame` 后，通过 `GameContext.engine` 驱动回合：`engine.advancePhase()` → 回合阶段自动推进 → AI 在 ACTION 阶段调用 `strategy.nextAction(state)` 产出 `ActionRequest` → `dispatch(state, req)` 执行。
...
| 需求 | 验收项 |
| --- | --- |
| FR3.5 | 触发型效果：在对应条件下自动触发（如「号召时」「回合开始时」），结算顺序符合 304.1.g 优先级 |
| FR3.5 | 常驻型效果：在有效区域内持续生效，战力增减实时反映在 `CardInstance.currentPower` |
| FR3.5 | 启动型效果：仅在 ACTION 阶段可用，效果文本描述的结算正确执行 |
| FR3.6 | 拦截（305.2）：战斗阶段每回合 1 次，可正常改目标；拦截后无法再次拦截 |
| FR3.6 | 连击（305.3）：连击角色每回合可攻击 2 次，第二次攻击的应对步骤正常 |
| FR3.6 | 强袭（305.4）：战胜时额外将冲击卡组顶 1 张放入时间线 |
| FR3.6 | 空袭（305.5）：R=0 角色可攻击任意破绽（含已有角色的战区） |
| FR3.6 | 唯一（305.6）：同名关键词角色在场时，无法号召第二个同名角色 |
| FR3.5 | 战力归零（301.16.b）：战力降至 0 的角色立即撤退，效果终止 |
| FR3.5 | 效果文本解析器（骨架）：`effect_json` 留空时不报错，当前端传入有效 JSON 时能正确结算 |
| NFR5 | 引擎代码注释标注规则条款编号（如 `// 304.1`），便于追溯 |
| NFR5 | Q&A 判例（Q1~Q9）覆盖：`EffectResolver` 中包含所有判例影响点的处理逻辑 |

...
> 前置：[迭代四](#06-详细设计-迭代四-引擎状态模型与回合流程.md) — `GameState`、`PlayerState`、`FieldZone`、`CardInstance`、`PhaseType`、`Zone` 等状态模型与回合流程已就绪
...
| `ActionTypeHandler` 统一操作处理接口 | 关键词能力的具体实现（迭代六） |
...
classDiagram
    ...
    class ActionDispatcher {
        -Map~ActionType, ActionTypeHandler~ handlers
        +register(ActionTypeHandler) void
    ...
    ActionTypeHandler <|.. BaseDeployHandler
    ActionTypeHandler <|.. SummonHandler
    ActionTypeHandler <|.. CombatBaseMoveHandler
    ActionTypeHandler <|.. ActivateEffectHandler
    ActionTypeHandler <|.. SetFaceDownHandler
    ActionTypeHandler <|.. FlipFaceUpHandler
    ActionTypeHandler <|.. AttachHandler
    ActionTypeHandler <|.. DetachHandler
    ActionTypeHandler <|.. EndPhaseHandler
    ActionDispatcher o--> ActionTypeHandler : registers
```
...
classDiagram
    ...
    ActionTypeHandler <|.. CombatAdjustHandler
    ActionTypeHandler <|.. AttackSequenceHandler
    ActionTypeHandler <|.. CombatResponseHandler
    ActionTypeHandler <|.. SurrenderHandler
...
> 以下接口由本迭代交付，供后续迭代（迭代六 效果系统、迭代七 对战 API）调用者使用。

| 公开方法 | 签名 | 调用方 | 用途 |
| --- | --- | --- | --- |
| `ActionDispatcher.dispatch(state, req)` | `ActionResult` | 迭代六/七 Service 层 | 执行操作 |
| `ActionDispatcher.handlerOf(type)` | `ActionTypeHandler` | 迭代六 AI LegalActionProvider | 复用 validate |
| `WinChecker.check(state)` | `String` | 迭代六 Service 层 | 效果结算后检查胜负 |
| `DistanceCalculator.distance(a, t)` | `int` | 迭代六 BattleHandler | 距离计算 |
| `DistanceCalculator.canReach(attacker, zone)` | `boolean` | 迭代六 Service 层 | 攻击可达性判断 |

> **挂接约定**：迭代六接入效果系统时，`EffectResolver` 在 `BattleResolver.resolve()` 中的战斗判定步骤后调用 `WinChecker.check()`。`ActivateEffectHandler.execute()` 骨架在迭代六填实后，替换为 `EffectResolver.resolve()` 调用。
...
| 需求 | 验收项 |
| --- | --- |
| FR3.3 | 基地部署：每阶段 1 次；基地区上限 6；部署后抽 1 张 |
| FR3.3 | 行动号召：每阶段 3 次（先攻首回合 1 次）；Lv4+ 正确校验撤退 Lv；同名唯一角色在场时禁止再次号召 |
| FR3.3 | 战基移动：每角色每回合 1 次；本回合进场者不能移动；战区↔基地区互换 |
| FR3.3 | 盖卡/翻开：正确翻转 isFaceDown 标记；翻开时清除 enteredThisTurn 标记 |
| FR3.3 | 结附/解除：结附卡按顺序叠放；父卡移动时结附卡跟随；解除后变回角色卡 |
| FR3.4 | 战斗判定：战力大者胜，败者撤退；相等相杀双方撤退 |
| FR3.4 | 攻击破绽：成功攻击破绽时冲击卡组顶 1 张放入时间线 |
| FR3.4 | 拦截：战斗阶段每回合 1 次；改攻击目标后战斗继续；拦截者本回合不能再拦截 |
| FR3.4 | 距离计算：`DistanceCalculator` 结果与距离矩阵一致 |
| FR3.4 | 胜负判定：时间线 ≥ 9 / 对方卡组抽空 → 立即结束；胜负标记正确 |
| FR3.7 | 认输：操作方认输，对手立即获胜 |
| NFR1 | 所有 Handler 在执行前调用 validate；不合规操作 → EngineException |
| NFR5 | 所有 Handler 注释标注规则条款编号（如 `// 303.2.a.3.1.1`） |

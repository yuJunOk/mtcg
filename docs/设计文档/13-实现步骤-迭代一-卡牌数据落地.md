...
1. axios 实例封装（baseURL `/api/v1`、请求/响应拦截器、统一错误处理）
2. 枚举字典工具（cardType/color/rarity/productType 的 code→desc 映射）
3. 通用分页表格组件（Element Plus Table + Pagination 封装）
4. 通用表单校验规则

### 设计意义

axios 封装统一处理 baseURL、token、错误码，业务代码只关心具体接口。响应拦截器识别 `code !== 0` 时用 ElMessage 弹错误提示，避免每个调用点重复写错误处理。枚举字典与后端枚举一一对应，用于列表展示中文标签与表单下拉选项，是前后端协同的关键约定。
...
export const ColorOptions = [
  { code: 'RED', desc: '红' }, { code: 'YELLOW', desc: '黄' }, { code: 'BLUE', desc: '蓝' },
  { code: 'GREEN', desc: '绿' }, { code: 'ORANGE', desc: '橙' }, { code: 'PURPLE', desc: '紫' }
]

export const RarityOptions = [
  { code: 'C', desc: '普通' }, { code: 'R', desc: '稀有' }, /* ... 12 种 */ 
]
...
// code -> desc 查询工具
export const codeToDesc = (options, code) =>
  options.find(o => o.code === code)?.desc ?? code
```
...
export const cardApi = {
  list: (params) => request.get('/cards', { params }),
  detail: (id) => request.get(`/cards/${id}`),
  create: (data) => request.post('/cards', data),
  update: (id, data) => request.put(`/cards/${id}`, data),
  remove: (id) => request.delete(`/cards/${id}`),
  batch: (data) => request.post('/cards/batch', { cards: data })
}
```

### 实现效果

- 所有接口调用统一走 `request` 实例，错误自动弹提示
- 枚举字典与后端 CHECK 约束完全一致，列表/表单可直接引用

...
| `mtcg-admin-web/` | ⛔ 待创建 | 前端管理后台目录不存在，需从零搭建 |
...
1. 在 `mtcg-admin-web` 目录用 Vite 创建 Vue 3 + TypeScript 项目
...
| 文件 | 操作 |
| --- | --- |
| `mtcg-admin-web/` | 新建整个工程 |
| `mtcg-admin-web/package.json` | 生成 |
| `mtcg-admin-web/vite.config.ts` | 配置 |
| `mtcg-admin-web/src/main.ts` | 入口 |
| `mtcg-admin-web/src/App.vue` | 根组件 |
| `mtcg-admin-web/src/router/index.ts` | 路由 |
| `mtcg-admin-web/src/layouts/MainLayout.vue` | 布局 |
| `mtcg-admin-web/src/views/DashboardView.vue` | 仪表盘占位 |
...
npm create vite@latest mtcg-admin-web -- --template vue-ts
cd mtcg-admin-web
npm install

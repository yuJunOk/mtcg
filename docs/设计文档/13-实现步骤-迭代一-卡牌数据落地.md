...
`mtcg-client` Monorepo 项目结构已搭建完成（详见 [概要设计 §7.3](./02-概要设计.md#73-前端项目结构mtcg-client)），包含三个包：
...
卡牌管理后台（CRUD 页面）后续可作为 `packages/admin-web` 添加到 Monorepo 中，复用 `packages/common` 的 API 层和类型定义。当前阶段，卡牌数据可通过 Swagger UI（`http://localhost:8081/api/swagger-ui.html`）或 curl 直接操作，无需前端页面即可完成步骤 8 的集成验证。

### 设计意义

Monorepo 结构让管理后台（admin-web）和游戏客户端（game-pc / game-mobile）共享同一套 API 封装和类型定义，避免重复维护。管理后台页面可后续按需添加，不阻塞当前迭代。
...
| --- | --- |
| `mtcg-client/` | ✅ 已创建 |
| `mtcg-client/packages/admin-web/` | 后续新建（卡牌管理 CRUD 页面） |

### 实现效果

- Monorepo 项目结构就绪，`packages/common` 的类型定义与后端 `GameState` 结构对齐
- 后续添加 `admin-web` 包时可直接复用 `@mtcg/common` 的 API 和类型
...
`packages/common` 已有基础 API 封装和类型定义，需补充：

1. 管理后台 API 方法（card CRUD、product CRUD），与游戏客户端 API（game actions）分开
2. 枚举字典：按后端枚举类逐一拆分文件，放入 `packages/admin-web/src/utils/enums/` 或 `packages/common/src/utils/enums/`（若管理后台和游戏客户端共用）
3. OpenAPI Generator 自动生成管理后台请求方法（可选，或手写）

### 设计意义

- **枚举拆分**：每个后端枚举类对应一个独立 TS 文件，与后端 `common/enums` 目录结构对齐
- **axios 封装**：管理后台和游戏客户端可共用 `packages/common` 的 axios 实例，也可各自扩展

### 涉及文件

| 文件 | 操作 |
| --- | --- |
| `packages/common/src/api/cardApi.ts` | 新建（卡牌 CRUD） |
| `packages/common/src/api/productApi.ts` | 新建（产品 CRUD） |
| `packages/admin-web/src/utils/enums/` | 后续新建（枚举字典） |
...
在 `packages/admin-web`（后续新建）中实现卡牌管理 CRUD 页面：
...
| 文件 | 操作 |
| --- | --- |
| `packages/admin-web/src/views/card/CardListView.vue` | 后续新建 |
| `packages/admin-web/src/views/card/CardFormDialog.vue` | 后续新建 |
| `packages/admin-web/src/views/card/CardDetailDrawer.vue` | 后续新建 |
| `packages/admin-web/src/views/card/BatchImportDialog.vue` | 后续新建 |

### 当前状态

卡牌管理前端页面暂未实现。当前阶段通过 Swagger UI 或 curl 直接操作后端 API 完成数据验证。页面实现将在后续迭代中作为 `packages/admin-web` 添加到 Monorepo。

### 检验方式

- Swagger UI 可完成卡牌 CRUD 全流程
- 后续前端页面实现后，表单校验与后端 CHECK 约束形成双重防线
...
在 `packages/admin-web`（后续新建）中实现产品管理 CRUD 页面：
...
| 文件 | 操作 |
| --- | --- |
| `packages/admin-web/src/views/product/ProductListView.vue` | 后续新建 |
| `packages/admin-web/src/views/product/ProductFormDialog.vue` | 后续新建 |
| `packages/admin-web/src/views/product/ProductDetailView.vue` | 后续新建 |

### 当前状态

产品管理前端页面暂未实现。当前阶段通过 Swagger UI 或 curl 直接操作后端 API。
...
| 3 | `packages/game-pc/` PC 端 | ✅ 已创建 | 4 |
| 4 | `packages/game-mobile/` 移动端 | ✅ 已创建 | 4 |
| 5 | `packages/common/src/api/cardApi.ts` | 新建 | 5 |
| 6 | `packages/common/src/api/productApi.ts` | 新建 | 5 |
| 7 | `packages/admin-web/` 管理后台 | 后续新建 | 6-7 |

前端共 7 项（4 已创建 + 2 待新建 + 1 后续新建）。

...
> 版本：v0.1
> 日期：2026-07-31
> 依据：[需求分析](./01-需求分析.md) FR6.1.1–FR6.1.5、[概要设计](./02-概要设计.md) §2 §5 §6 §9（D3）、[迭代四](./06-详细设计-迭代四-引擎状态模型与回合流程.md)、[迭代五](./07-详细设计-迭代五-引擎行动与战斗.md)、[迭代六](./08-详细设计-迭代六-效果系统与关键词能力.md)
...
| 操作合法优先 | AI 产出的每个 `ActionRequest` 必须能通过 `ActionTypeHandler.validate`；不合法操作由 `LegalActionProvider` 提前过滤，不依赖引擎异常兜底 |
...
| `ActionTypeHandler.validate(state, request)` | AI 枚举合法操作时复用 |
...
策略选择的前提是枚举当前阶段所有合法操作。`LegalActionProvider` 复用引擎 `ActionTypeHandler.validate` 进行过滤，避免在 AI 侧重复规则逻辑（NFR5 规则可追溯）。
...
public class LegalActionProvider {
    ...
    private boolean isLegal(GameState state, ActionRequest req) {
        try {
            ActionTypeHandler handler = dispatcher.handlerOf(req.getType());
...
sequenceDiagram
    ...
    participant H as ActionTypeHandler
...
> 以下接口由本迭代交付，供后续迭代（迭代八 排位系统）调用者使用。

| 公开方法 | 签名 | 调用方 | 用途 |
| --- | --- | --- | --- |
| `AIManager.createGame(deckId, aiConfig)` | `GameStateVO` | 迭代八 MatchmakingManager | 超时填补对手 |
| `AIManager.getDecisionLogs(gameId)` | `List<AIDecisionLog>` | 迭代八 排行榜展示 | AI 决策复盘 |
| `AIPlayer.isAI()` | `boolean` | 迭代八 排位系统 | 区分 AI 对局与真人 |
| `AIStrategy.nextAction(state)` | `ActionRequest` | 迭代八 排行榜 | 局面评分辅助展示 |
...
| 需求 | 验收项 |
| --- | --- |
| FR6.1 | AI 对局可正常完成：创建 → Mulligan → 行动 → 战斗 → 胜负判定 |
| FR6.1 | AI 决策合法：所有 `ActionRequest` 通过 `ActionTypeHandler.validate` |
| FR6.1 | AI 难度分级：EASY/MEDIUM/HARD 三档决策质量有差异（通过 `BoardEvaluator` 分差验证） |
| FR6.1 | AI 决策可观测：`AIDecisionLog` 记录评估分、候选操作、最终选择 |
| FR6.1 | 启发式策略：基于规则打分（场面优势 > 战力差 > 手牌数） |
| FR6.1 | LLM 策略骨架：接口存在，调用时返回空/降级到启发式 |
| FR6.1 | HumanPlayer / AIPlayer 互换：对 `GameService` 透明，无需修改 |
| FR6.1 | 同进程调用：AI 代码在 Spring 进程内，不走 HTTP/RPC |
| FR6.1 | AI 不作弊：不读取对手手牌内容；仅统计公开量（手牌数/卡组剩余/场上信息） |
| NFR7 | AI 对局同样写 `action_log`，AI 决策内容以特殊 ActionType 标记 |

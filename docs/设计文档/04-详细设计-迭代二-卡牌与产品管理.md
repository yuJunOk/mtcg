...
| ProductDO / ProductMapper / ProductService / ProductController | 对战 API（迭代七） |
...
| 枚举 | 迭代一 | `EnumCardType` / `EnumColor` / `EnumRarity`（迭代一已建好） |
...
> 产品即卡包/商品系列目录（如 BP01 复仇者联盟补充包、SD01 蜘蛛侠预组牌），不需要单独分类表。建表 SQL 已在迭代一 init.sql 中统一执行，此处仅作字段说明。
...
| `init.sql` | `resources/db` | 建表 SQL（含 card + product 表） |
...
| FR1.3 | 批量导入：成功/失败记录分开统计；部分失败不影响成功记录 |
...
| FR1.2 | 产品下的卡牌查询（`/admin/products/{productCode}/cards`）正确 |

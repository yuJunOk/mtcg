...
> 版本：v0.3
> 日期：2026-08-11
> 状态：✅ 已实现（冲突以 as-built 与代码为准）
> 依据：[需求分析](./01-需求分析.md) FR1.1–FR1.5、[概要设计](./02-概要设计.md)、[迭代一详细设计](./03-详细设计-迭代一-用户系统与用户管理.md)

---

## 0. As-built 摘要（实现与原稿差异）

| 项 | 原稿 | 实际交付 |
| --- | --- | --- |
| 特征 | `mtcg_card.traits` CSV 为主 | `mtcg_card_feature` + `mtcg_card_feature_rel`；`CardFeatureSelector`；`traits` 列兼容保留 |
| 控制器 | `PublicCardController` 等命名 | `CardController` / `ProductController` + `Admin*` |
| 按产品查卡 | `/admin/products/{code}/cards` | 公开 `GET /products/{productCode}/cards` |
| 批量导入 | `POST /cards/batch` | **未做，延后** |
| Schema | `init.sql` | `sql/init-schema.sql` / `init-data.sql` |
| 审计 | 无 | 前置 `mtcg_audit_log` + `AuditService` |
| 管理端 | — | `MtcgDialog`、`ProductSelector`、`CardFeatureSelector`（`el-splitter`） |
| Tag | 概要曾列 `/tags` | **不做**，以 CardFeature 替代 |
...
在迭代一（用户系统 + 鉴权基础设施）基础上，实现卡牌与产品数据的完整 CRUD 和查询功能，对接管理后台卡牌/产品/特征管理页面。

### 1.2 范围

| 包含 | 不包含 |
| --- | --- |
| CardDO / ProductDO 及 Mapper、Service、Controller | 卡组构筑（迭代三） |
| 卡牌 CRUD + 条件查询 | 引擎（迭代四） |
| 产品 CRUD + 按产品筛选卡牌 | 对战 API（迭代七） |
| CardFeature CRUD + 卡牌关联 | 效果系统（迭代六） |
| 管理后台卡牌/产品/特征页 + 选择器 | **卡牌批量导入（延后）** |
| `@RequireRole({CARD_ADMIN, SYS_ADMIN})` 鉴权 | 独立 Tag 模块 |

### 1.3 关键决策

| 编号 | 决策 | 理由 |
| --- | --- | --- |
| D1 | 枚举字段在 DO 层用 String + MyBatis-Flex TypeHandler | MyBatis-Flex 对泛型枚举支持弱，直接存 String 最稳定 |
| D2 | `effect_text` / `effect_json` 双字段分离存储 | 文本展示用 `effect_text`；结构化解析（迭代六）用 `effect_json`；当前 `effect_json` 留空 |
| D3 | `image_path` 存相对路径而非 URL | 静态资源由 Nginx/CDN 托管，前端拼接 baseUrl；避免硬编码 |
| D4 | 产品编号/卡牌编号用 VARCHAR UNIQUE 约束 | Service 层校验唯一性 + DB UNIQUE 双重保证，防止脏数据 |
| D5 | 特征用独立表 + 关联表 | 可维护字典、多对多；`traits` 仅兼容 |
| D6 | 批量导入延后 | 单条录入 + 特征选择已闭环 |
...
| 枚举 | 迭代一 | `EnumCardType` / `EnumColor` / `EnumRarity` |
| 建表 | 本迭代 | `init-schema.sql`（card / product / feature / audit） |
...
CREATE TABLE mtcg_card (
    ...
    traits          VARCHAR(256),                               -- 【兼容】CSV；主路径见 mtcg_card_feature*
...
> 产品即卡包/商品系列目录（如 BP01 复仇者联盟补充包、SD01 蜘蛛侠预组牌），不需要单独分类表。建表 SQL 见 `sql/init-schema.sql`，此处仅作字段说明。
...
> update_time 自动更新：mtcg_card 与 mtcg_product 共用 §2.1 定义的 `update_update_time()` 触发器函数。
...
- String environment      // 环境，如 S1；冲击卡留空
- String traits
...
> **状态：延后。** 验收不以本 DTO / `batchImport` 为准。
...
路径：/api/v1/admin/cards
类注解：@RequireRole({EnumUserRole.CARD_ADMIN, EnumUserRole.SYS_ADMIN})  // 写操作需管理员
```

| 方法 | HTTP | 路径 | 入参 | 出参 | 说明 |
| --- | --- | --- | --- | --- | --- |
| queryCards | GET | `/admin/cards` | CardQueryDTO（Query Param） | `Result<Page<CardVO>>` | 分页查询 |
| getCard | GET | `/admin/cards/{id}` | Long id | `Result<CardVO>` | 查询详情 |
| createCard | POST | `/admin/cards` | @RequestBody CardCreateDTO | `Result<Long>` | 返回新增 ID |
| updateCard | POST | `/admin/cards/{id}` | Long id, @RequestBody CardUpdateDTO | `Result<Void>` | |
| deleteCard | POST | `/admin/cards/{id}/delete` | Long id | `Result<Void>` | |

> **接口设计说明**：所有写操作统一使用 POST，通过路径区分操作类型（`POST /{id}` 为更新，`POST /{id}/delete` 为删除）。
...
| 方法 | HTTP | 路径 | 入参 | 出参 | 说明 |
| --- | --- | --- | --- | --- | --- |
| listProducts | GET | `/admin/products` | 无 | `Result<List<ProductVO>>` | 产品列表 |
| getProduct | GET | `/admin/products/{id}` | Long id | `Result<ProductVO>` | 查询详情 |
| createProduct | POST | `/admin/products` | @RequestBody ProductCreateDTO | `Result<Long>` | 返回新增 ID |
| updateProduct | POST | `/admin/products/{id}` | Long id, @RequestBody ProductCreateDTO | `Result<Void>` | |
| deleteProduct | POST | `/admin/products/{id}/delete` | Long id | `Result<Void>` | |
| listCardsByProduct | GET | `/admin/products/{productCode}/cards` | String productCode, Integer page, Integer size | `Result<Page<CardVO>>` | 按产品筛选卡牌 |
...
| `BatchImportDTO.java` | `domain.dto` | 批量导入 DTO（**延后，未交付**） |
| `BatchImportResult.java` | `domain.dto` | 批量导入结果（**延后，未交付**） |
...
| `ProductVO.java` | `domain.vo` | 产品返回视图 |
| `ProductController.java` | `controller` | 产品 REST API |
| `GlobalExceptionHandler.java` | `advice` | 全局异常处理 |
| `init-schema.sql` | `resources/sql` | 建表 SQL（card / product / feature / audit） |
...
> 以下接口由本迭代交付，供后续迭代（迭代三~九）调用者使用。

| 公开方法 | 签名 | 调用方 | 用途 |
| --- | --- | --- | --- |
| CardMapper | `BaseMapper<CardDO>` | 迭代三 DeckService（卡组校验） | 按 cardCode 批量查询卡牌 |
| CardService | `CardVO getCardById(Long)` | 迭代三 DeckService（收藏联查） | 单卡详情 |
| ProductService | `ProductVO getProductById(Long)` | 迭代三 DeckService（产品管理） | 单产品详情 |
| CardMapper | `List<CardDO> selectListByIds(List<Long>)` | 迭代七 GameService（加载快照） | 按 ID 批量查询 |
...
| 需求 | 验收项 |
| --- | --- |
| FR1.1 | 卡牌 CRUD：创建 / 查询 / 编辑 / 删除正常；编号重复创建返回 3002 |
| FR1.1 | 产品 CRUD：创建 / 查询 / 编辑 / 删除正常；编号重复创建返回 3004 |
| FR1.2 | 卡牌条件查询：按名称模糊 / 类型 / 颜色 / 稀有度 / 产品编号筛选正确 |
| FR1.2 | 产品列表：返回 productName / releaseDate / description 字段正确 |
| FR1.3 | ~~批量导入~~ → **延后**；改为验收：特征 CRUD + 卡牌多选关联正常 |
| FR1.1 | PLAYER 角色访问 `/admin/cards` 或 `/admin/products` → 403 无操作权限 |
| FR1.1 | 字段校验：必填字段为空 → 400 参数错误 |
| FR1.1 | 枚举字段非法值（如 color=INVALID）→ 400 参数错误 |
| FR1.1 | CARD_ADMIN 角色可正常操作卡牌/产品管理接口 |
| FR1.1 | `CardDO` 与 `CardVO` 字段映射一致，无 passwordHash 等敏感字段暴露 |
| FR1.2 | 产品下的卡牌查询（`GET /products/{productCode}/cards`）正确 |
| — | 管理端 `ProductSelector` / `CardFeatureSelector` 可完成产品与特征选择 |

...
CREATE TABLE "mtcg_user" (
    ...
    usercode        VARCHAR(32)     NOT NULL,                          -- 玩家编号（登录凭证，唯一）
    password_hash   VARCHAR(72)     NOT NULL,                          -- BCrypt 密文（60 字符，72 留余量）
    username        VARCHAR(64),                                       -- 用户名（展示名，可空）
...
| --- | --- | --- |
| id | BIGSERIAL | 主键，自增 |
| usercode | VARCHAR(32) | 玩家编号，系统自动生成（100000 + 自增 ID），唯一，登录凭证，不可改 |
| password_hash | VARCHAR(72) | BCrypt 密文，永不明文存储、永不返回给前端 |
| username | VARCHAR(64) | 用户名，展示名，可空，可修改 |
| avatar | VARCHAR(256) | 头像 URL，迭代九完善头像系统 |
...
public class UserDO {
    ...
    private Long id;
    private String usercode;        // 玩家编号，登录凭证
    private String passwordHash;   // BCrypt 密文，永不返回前端
    private String username;         // 用户名，展示名
...
| DTO | 用途 | 字段 |
| --- | --- | --- |
| `UserRegisterDTO` | 注册请求 | usercode(@NotBlank @Length 3-32)、password(@NotBlank @Length 6-32)、username(@Length max 64) |
| `UserLoginDTO` | 登录请求 | usercode(@NotBlank)、password(@NotBlank) |
| `UserUpdateDTO` | 修改个人资料 | username(@Length max 64)、avatar(@Length max 256) |
| `ChangePasswordDTO` | 修改密码 | oldPassword(@NotBlank)、newPassword(@NotBlank @Length 6-32) |
| `UserQueryDTO` | 管理员列表查询 | usercode(模糊)、username(模糊)、role(精确)、status(精确)、page=1、size=20 |
| `AdminUserCreateDTO` | 管理员新增用户 | password(@NotBlank)、username、role(@NotBlank)（usercode 系统自动生成） |
| `AdminUserUpdateDTO` | 管理员修改用户 | username、role、status |

```java
// UserRegisterDTO 示例（其余同理）
@Data
public class UserRegisterDTO {
    @NotBlank(message = "玩家编号不能为空")
    @Length(min = 3, max = 32, message = "玩家编号长度需为 3-32")
    ...
    @Length(max = 64, message = "用户名长度不能超过 64")
...
| `UserVO` | 用户视图（不含密码） | id、usercode、username、avatar、role、status、createTime |

```java
@Data
public class UserVO {
    private Long id;
    private String usercode;    // 玩家编号
...
public class JwtManager {
    ...
    public static final String CLAIM_USERCODE = "usercode";
    ...
    /** 生成 Token */
    public String generateToken(Long userId, String usercode, String role) {
        long now = System.currentTimeMillis();
        long expireAt = now + jwtProperties.getExpireMinutes() * 60_000L;
        return Jwts.builder()
                .claim(CLAIM_USER_ID, userId)
                .claim(CLAIM_USERCODE, usercode)
...
| 方法 | 逻辑 |
| --- | --- |
| register | 1. 按 usercode 查重 → 已存在抛 `玩家编号已存在`<br>2. BCrypt 加密密码<br>3. 组装 UserDO（role=PLAYER、status=ACTIVE）<br>4. insert → 返回 id |
| login | 1. 按 usercode 查用户 → 不存在抛 `玩家编号或密码错误`<br>2. 校验 status=ACTIVE → 否则抛 `账号已被禁用`<br>3. BCrypt 校验密码 → 不匹配抛 `玩家编号或密码错误`<br>4. JwtManager 生成 token<br>5. 组装 LoginVO 返回 |
| getCurrentUserInfo | selectOneById → DO → VO |
| updateCurrentUser | selectOneById 校验存在 → 仅拷贝 username/avatar 非空字段 → updateById |
| changePassword | 1. selectOneById 校验存在<br>2. BCrypt matches 校验原密码 → 不匹配抛 `原密码错误`<br>3. encode 新密码 → updateById |
| listUsers | 1. 构造 QueryWrapper（usercode LIKE / username LIKE / role 精确 / status 精确）<br>2. 按 create_time 倒序<br>3. paginate 分页<br>4. DO → VO 列表 |
| getUserById | selectOneById → 不存在抛 `用户不存在` → VO |
| adminCreateUser | 1. usercode 查重<br>2. 校验 role 合法（EnumUserRole.of）<br>3. BCrypt 加密密码<br>4. 组装 UserDO（指定 role、status=ACTIVE）<br>5. insert |
| adminUpdateUser | 1. selectOneById 校验存在<br>2. 禁止修改 SYS_ADMIN 的 role（防降级锁死）<br>3. 拷贝 username/role/status 非空字段<br>4. updateById |
...
public class UserServiceImpl implements UserService {
    ...
    public Long register(UserRegisterDTO dto) {
        ...
        // 插入后根据自增 ID 生成 usercode（固定基数 100000，ID=1 → usercode="100001"）
        userDO.setUsercode(String.valueOf(100000L + userDO.getId()));
    ...
    public LoginVO login(UserLoginDTO dto) {
        UserDO userDO = userMapper.selectOneByQuery(
                QueryWrapper.create().eq("usercode", dto.getUsercode()));
        if (userDO == null) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "玩家编号或密码错误");
        }
        ...
        String token = jwtManager.generateToken(userDO.getId(), userDO.getUsercode(), userDO.getRole());
    ...
    public PageVO<UserVO> listUsers(UserQueryDTO query) {
        ...
        if (query.getUsercode() != null && !query.getUsercode().isBlank()) {
            qw.like("usercode", query.getUsercode());
        }
    ...
    private UserVO toVO(UserDO userDO) {
        ...
        vo.setUsercode(userDO.getUsercode());
    ...
    private boolean existsByUsercode(String usercode) {
        return userMapper.selectOneByQuery(
                QueryWrapper.create().eq("usercode", usercode)) != null;
    }
...
| --- | --- | --- |
| 注册玩家编号已存在 | PARAMS_ERROR(400) | 玩家编号已存在 |
| 登录玩家编号/密码错误 | PARAMS_ERROR(400) | 玩家编号或密码错误 |
...
public class JwtInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
        ...
        request.setAttribute(SecurityConstant.ATTR_USERCODE, userDO.getUsercode());

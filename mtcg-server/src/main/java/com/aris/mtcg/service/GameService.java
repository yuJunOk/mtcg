...
public interface GameService {
    ...
    /** 创建对局或房间（FR4.1），返回对外业务编码 gameCode */
    String createGame(Long userId, GameCreateDTO dto);

    /** 加入等待中的房间，开局后返回 gameCode（id 可为数字主键或 G- 编码） */
    String joinGame(Long userId, String idOrCode, GameJoinDTO dto);

    /** 在线匹配：有空房则加入并开局，否则 matched=false */
    GameMatchVO matchGame(Long userId, GameJoinDTO dto);

    /** 取消本人发起的等待房间（id 可为数字主键或 G- 编码） */
    void cancelWaiting(Long userId, String idOrCode);

    /** 查询对局状态（FR4.2），含隐私裁剪 */
    GameStateVO getGameState(Long userId, String idOrCode);

    /** 执行操作（FR4.3） */
    ActionResultVO executeAction(Long userId, String idOrCode, ActionRequestDTO dto);

    /** 认输（FR4.4） */
    void surrender(Long userId, String idOrCode);

    /** 复盘数据（FR4.5） */
    ReplayVO getReplay(String idOrCode);
